    ...
    if historial.size > 0
      ...
      historial.each{|reg|
          ...
          "nota" => "<b>#{"%002i"%reg.nota_final}</b>"
			...
			pdf.text "\n"
      pdf.text to_utf16("Esta constancia se expide a solicitud de la parte interesada. En Caracas, a los #{t.day} días del mes de #{mes(t.month)} de #{t.year}."), :font_size => 11, :justification => :full
      pdf.text "\n"
      pdf.text "\n"
      pdf.text "\n"
      pdf.text "\n"
